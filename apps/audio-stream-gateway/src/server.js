...
const { MIN_CACHED_BYTES, cookiesPathFor, buildObjectKey, buildYtDlpArgs } = trackCache;
...
app.get('/stream/:videoId', auth.requireTenant, async (req, res, next) => {
  ...
  try {
    ...
    trackCache
      .cacheTrack({ videoId, tenant, store, cacheKey })
      .then(async (result) => {
        if (result.ok) {
          cachingJobs.delete(jobKey);
          return;
        ...
        // YouTube refuses these downloads from cloud egress but not from a home
        // connection, so a failure here is not necessarily the end of it: record what was
        // wanted and let the local fetch worker pick it up.
        await queue.enqueue(store, videoId).catch((error) => {
          console.error(`[Queue] Could not record ${videoId} as wanted`, error);
        });
